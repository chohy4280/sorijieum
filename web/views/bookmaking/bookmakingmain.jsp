...
function confirmExit() {
  if (checkUnload) return "페이지에서 나가시면 도서제작에 패널티가 있을 수 있습니다.";
}; */ 
	...
	var second = 0;
		...
		timer = setInterval(function() {
			...
			if (second < 2 && minute == 0 && hour == 0) {
				...
				checkUnload = false;
				alert("제작자 " + id + "님, 도서 제작시간이 종료되어 도서제작 메인페이지로 이동됩니다.");
				...
				<table class="table">
					<tr style="background-color: #f5f5f5">
						...
						<td width="40%" align="center">
							...
							<div class="ui yellow button" id="ui-tooltip-content<%= i %>">
								...
								onclick="javascript:complete(<%= loginMember.getUserId() %>)" disabled="disabled">
								제출</button>
							<script type="text/javascript">
					...
					<tr style="background-color: #f5f5f5">
						...
						<textarea id="<%=bmp.getBookCode()%>_<%= i %>" name="<%=bmp.getBookCode()%>_<%= i %>" cols="66" rows="6"
							...
							id="<%=i%>_pagenum" min="0" inputmode="numeric"
				...
				<table class="table">
					<tr style="background-color: #f5f5f5">
						...
						<td width="40%" align="center">
							...
							<div class="ui yellow button" id="ui-tooltip-content<%= i %>">
								<div class="ui custom popup transition hidden" id="guide<%= i+1 %>"
									style="padding: 24px; max-width: 500px; margin-top: 25px;

...
/**
 * Servlet implementation class BookMakingListServlet
 */
@WebServlet("/bmlist")
public class BookMakingListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public BookMakingListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//제작중 도서 더보기 리스트 처리용 컨트롤러 - 페이징 처리함
		int currentPage = 1;
		...
		int listCount = bservice.getListCountMaking(); //테이블의 전체 목록 갯수 조회
		//총 페이지수 계산
		...
		if(list.size() > 0 && makelist.size() > 0) {
			...
			request.setAttribute("message", "제작중 도서 전체 목록  조회 실패!");
			view.forward(request, response);
		}
	}

...
/**
 * Servlet implementation class userReadBookListServlet
 */
@WebServlet("/urblist.ad")
public class AdUserReadBookListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdUserReadBookListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// ----------------- 페이징 처리 추가 -----------------
			...
			//전체 게시글 수 조회
			int listCount  = new MyBookService().getListCountAdmin(userid);
			...
			ArrayList<adminMyBook> list = new MyBookService().selectUserReadbookList(userid, startRow, endRow);
			
			
			RequestDispatcher view = null;
			
			if(list.size() >= 0) {
				view = request.getRequestDispatcher("views/admin/UserMyBook.jsp");;
				...
				request.setAttribute("message", "이용자 읽은 도서 상세정보 조회 실패!");
			}
